...
def main():
    ...
    def update():
        ...
        if fSim_d.getResultFirst():
            ...
            dx = x[1] - x[0]
            idx = int(fSim_d.times[0, 0] / n.max(fSim_d.times) * 999)
            ax[0, 0].set_title(f't={fSim_d.times[0, 0]}')
            ax[0, 0].plot(x, data[0, 1:], c='deepskyblue', ls=':', label='FD-')
            ax[0, 0].legend()
            idx = int(fSim_d.times[0, 1] / n.max(fSim_d.times) * 999)
            ax[0, 1].set_title(f't={fSim_d.times[0, 1]}')
            ax[0, 1].plot(x, data[333, 1:], c='deepskyblue', ls=':', label='FD-')
            ax[0, 1].legend()
            idx = int(fSim_d.times[1, 0] / n.max(fSim_d.times) * 999)
            ax[1, 0].set_title(f't={fSim_d.times[1, 0]}')
            ax[1, 0].plot(x, data[666, 1:], c='deepskyblue', ls=':', label='FD-')
            ax[1, 0].legend()
            idx = int(fSim_d.times[1, 1] / n.max(fSim_d.times) * 999)
            ax[1, 1].set_title(f't={fSim_d.times[1, 1]}')
            ax[1, 1].plot(x, data[999, 1:], c='deepskyblue', ls=':', label='FD-')
            ax[1, 1].legend()
            plt.tight_layout()
            fig.canvas.draw()
        ...
        if sSim_u.getResultFirst():
            ...
            dx = x[1] - x[0]
            idx = int(sSim_u.times[0, 0] / n.max(sSim_u.times) * 999)
            ax[0, 0].set_title(f't={sSim_u.times[0, 0]}')
            ax[0, 0].plot(x, data[idx, 1:], c='coral', ls='--', label='S+')
            ax[0, 0].legend()
            idx = int(sSim_u.times[0, 1] / n.max(sSim_u.times) * 999)
            ax[0, 1].set_title(f't={sSim_u.times[0, 1]}')
            ax[0, 1].plot(x, data[idx, 1:], c='coral', ls='--', label='S+')
            ax[0, 1].legend()
            idx = int(sSim_u.times[1, 0] / n.max(sSim_u.times) * 999)
            ax[1, 0].set_title(f't={sSim_u.times[1, 0]}')
            ax[1, 0].plot(x, data[idx, 1:], c='coral', ls='--', label='S+')
            ax[1, 0].legend()
            idx = int(sSim_u.times[1, 1] / n.max(sSim_u.times) * 999)
            ax[1, 1].set_title(f't={sSim_u.times[1, 1]}')
            ax[1, 1].plot(x, data[idx, 1:], c='coral', ls='--', label='S+')
            ax[1, 1].legend()
            plt.tight_layout()
            fig.canvas.draw()
            ...
            err = n.zeros(1000)
            for i in range(1000):
                err[i] = n.sum((data[i, 1:] - sSim_u.T_vals) ** 2)
            t = n.linspace(0, n.max(sSim_u.times), 1000)
            ax_err_d.plot(t, err * dx, c='coral', label='S+ error')
            ax_err_d.legend(loc=(0.9, 0.7))
            fig_err.canvas.draw()
        ...
        if sSim_d.getResultFirst():
            ...
            dx = x[1] - x[0]
            idx = int(sSim_d.times[0, 0] / n.max(sSim_d.times) * 999)
            ax[0, 0].set_title(f't={sSim_d.times[0, 0]}')
            ax[0, 0].plot(x, data[idx, 1:], c='deepskyblue', ls=':', label='S-')
            ax[0, 0].legend()
            idx = int(sSim_d.times[0, 1] / n.max(sSim_d.times) * 999)
            ax[0, 1].set_title(f't={sSim_d.times[0, 1]}')
            ax[0, 1].plot(x, data[idx, 1:], c='deepskyblue', ls=':', label='S-')
            ax[0, 1].legend()
            idx = int(sSim_d.times[1, 0] / n.max(sSim_d.times) * 999)
            ax[1, 0].set_title(f't={sSim_d.times[1, 0]}')
            ax[1, 0].plot(x, data[idx, 1:], c='deepskyblue', ls=':', label='S-')
            ax[1, 0].legend()
            idx = int(sSim_d.times[1, 1] / n.max(sSim_d.times) * 999)
            ax[1, 1].set_title(f't={sSim_d.times[1, 1]}')
            ax[1, 1].plot(x, data[idx, 1:], c='deepskyblue', ls=':', label='S-')
            ax[1, 1].legend()
            plt.tight_layout()
            fig.canvas.draw()
            ...
            err = n.zeros(1000)
            for i in range(1000):
                err[i] = n.sum((data[i, 1:] - sSim_d.T_vals) ** 2)
            t = n.linspace(0, n.max(sSim_d.times), 1000)
            ax_err_d.plot(t, err * dx, c='deepskyblue', label='S- error')
            ax_err_d.legend(loc=(0.9, 0.7))
            fig_err.canvas.draw()
